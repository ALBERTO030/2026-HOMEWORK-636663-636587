...
class BorsaTest {
	...
	// --------------------------------------------------------
	@Test
	void testHasAttrezzoEsiste() {
		Borsa borsa = new Borsa();
		Attrezzo torcia = new Attrezzo("libro", 2);
		borsa.addAttrezzo(torcia);
		
		assertTrue(borsa.hasAttrezzo("libro"), "Dovrebbe trovare l'attrezzo inserito");
		}

	@Test
	void testHasAttrezzoNonEsiste() {
		Borsa borsa = new Borsa();
		assertFalse(borsa.hasAttrezzo("osso"), "Non dovrebbe trovare un attrezzo mai inserito");
	}
	...
	void testRemoveAttrezzo() {
		Borsa borsa = new Borsa();
		Attrezzo cuffie = new Attrezzo("cuffie", 2);
		borsa.addAttrezzo(cuffie);
		
		// Rimuovo l'attrezzo
		Attrezzo rimosso = borsa.removeAttrezzo("cuffie");
		
		assertEquals(cuffie, rimosso, "Il metodo dovrebbe restituirmi le cuffie rimosse");
